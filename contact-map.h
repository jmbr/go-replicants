...
extern struct contact_map *new_contact_map(const struct protein *protein, double d_max);
...
extern double contact_map_get_distance(const struct contact_map *self, size_t i, size_t j);

extern int contact_map_plot(const struct contact_map *self);
#endif // !CONTACT_MAP_H
